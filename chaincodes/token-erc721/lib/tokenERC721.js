...
class TokenERC721Contract extends Contract {
    ...
    */
    ...
    async SetOption(ctx, name, symbol) {
        ...
        // Check minter authorization - this sample assumes fer is the issuer with privilege to set the name and symbol
    ...
    */
    async MintWithTokenURI(ctx, tokenId, tokenURI) {
        ...
        // Check minter authorization - this sample assumes fer is the issuer with privilege to mint a new token
        const clientMSPID = ctx.clientIdentity.getMSPID();
        if (clientMSPID !== 'ferMSP') {
