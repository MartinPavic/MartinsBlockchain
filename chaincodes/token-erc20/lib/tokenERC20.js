...
class TokenERC20Contract extends Contract {
    ...
    */
    ...
    async Mint(ctx, amount) {
        ...
        // Check minter authorization - this sample assumes fer is the central banker with privilege to mint new tokens
        const clientMSPID = ctx.clientIdentity.getMSPID();
        if (clientMSPID !== 'ferMSP') {
    ...
    async Burn(ctx, amount) {
        ...
        // Check minter authorization - this sample assumes fer is the central banker with privilege to burn tokens
        const clientMSPID = ctx.clientIdentity.getMSPID();
        if (clientMSPID !== 'ferMSP') {
            throw new Error('client is not authorized to mint new tokens');
        }
