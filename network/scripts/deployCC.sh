...
CC_NAME=${1}
CC_SRC_PATH="../chaincodes/token-$1"
...
approveForMyOrg() {
  ...
  setGlobals $ORG
  set -x
  peer lifecycle chaincode approveformyorg -o localhost:7050 --ordererTLSHostnameOverride orderer.unizg.hr --tls --cafile "$ORDERER_CA" --channelID $CHANNEL_NAME --name ${CC_NAME} --version ${CC_VERSION} --package-id ${PACKAGE_ID} --sequence ${CC_SEQUENCE} ${INIT_REQUIRED} ${CC_END_POLICY} ${CC_COLL_CONFIG} >&log.txt
  res=$?
  { set +x; } 2>/dev/null
  cat log.txt
...
commitChaincodeDefinition() {
  ...
  set -x
  peer lifecycle chaincode commit -o localhost:7050 --ordererTLSHostnameOverride orderer.unizg.hr --tls --cafile "$ORDERER_CA" --channelID $CHANNEL_NAME --name ${CC_NAME} "${PEER_CONN_PARMS[@]}" --version ${CC_VERSION} --sequence ${CC_SEQUENCE} ${INIT_REQUIRED} ${CC_END_POLICY} ${CC_COLL_CONFIG} >&log.txt
  res=$?
  { set +x; } 2>/dev/null
  cat log.txt
...
infoln "Installing chaincode on peer0.fer..."
installChaincode fer
infoln "Install chaincode on peer0.fsb..."
installChaincode fsb
...
checkCommitReadiness fer "\"ferMSP\": true" "\"fsbMSP\": false"
checkCommitReadiness fsb "\"ferMSP\": true" "\"fsbMSP\": false"
...
checkCommitReadiness fer "\"ferMSP\": true" "\"fsbMSP\": true"
checkCommitReadiness fsb "\"ferMSP\": true" "\"fsbMSP\": true"

## now that we know for sure both orgs have approved, commit the definition
commitChaincodeDefinition fer fsb
